    ...
    public class UserService : IUserService
    {
        ...
        public async Task<User?> VerifyAndGetUserAsync(UserLoginDTO credentials)
        {
            ...
            try
            {
                ...
                logger.LogInformation("User authorized: {Username}", credentials.Username!);
            }
            catch (EntityNotAuthorizedException ex)
            {
                logger.LogError("Authorization failed for user {Username}: {Message}",
                    credentials.Username, ex.Message);
            }
            return user;
        }
    }
